...
from datamigration_utils.sf_connector_utils import SfConnectorUtils
...
REFRESH_AUTH_DAG_TASK = "refresh_auth"
MIGRATE_DATA_DAG_TASK = "migrate_dara"
...
sf_connector = SfConnectorUtils(SF_CONNECTOR_HOST)
...
def _migrate_data(ti) -> None:
    ...
    response = sf_connector.migrate_data(sf_migration_config)
...
with models.DAG(
    ...
    refresh_auth = PythonOperator(
        task_id=REFRESH_AUTH_DAG_TASK,
        python_callable=_refresh_auth,
        dag=dag,
    )
    ...
    (get_config >> get_valid_tables >> refresh_auth >> migrate_data >> dag_report)
